(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const a of document.querySelectorAll('link[rel="modulepreload"]'))o(a);new MutationObserver(a=>{for(const c of a)if(c.type==="childList")for(const v of c.addedNodes)v.tagName==="LINK"&&v.rel==="modulepreload"&&o(v)}).observe(document,{childList:!0,subtree:!0});function r(a){const c={};return a.integrity&&(c.integrity=a.integrity),a.referrerpolicy&&(c.referrerPolicy=a.referrerpolicy),a.crossorigin==="use-credentials"?c.credentials="include":a.crossorigin==="anonymous"?c.credentials="omit":c.credentials="same-origin",c}function o(a){if(a.ep)return;a.ep=!0;const c=r(a);fetch(a.href,c)}})();const T="https://www.bortakvall.se/api",O="/products",M="/orders",x=async()=>{const t=await fetch(`${T}${O}`);if(!t.ok)throw document.querySelector("#nav-output").innerHTML=`<h2 class="nav-item px-2">${t.status} ${t.statusText}</h2>`,new Error(`Could not get data, reason: ${t.status} ${t.statusText}`);return await t.json()},A=async t=>{const e=await fetch(`${T}${M}`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(t)});if(!e.ok)throw new Error(`${e.status} ${e.statusText}`);return await e.json()},s=t=>document.querySelector(`${t}`),l=t=>s(t).classList.add("d-none"),d=t=>s(t).classList.remove("d-none"),P=s(".customer-details"),b=s("#customer-first-name"),k=s("#customer-last-name"),y=s("#customer-address"),h=s("#customer-postal-number"),w=s("#customer-city"),$=s("#customer-phone"),L=s("#customer-email");let m,f=localStorage.getItem("Shopping cart")??"[]",i=JSON.parse(f),H=localStorage.getItem("Total price")??"0",p=JSON.parse(H),R=localStorage.getItem("Customer data")??"[]",n=JSON.parse(R);const q=()=>{s("#cart-item-count").textContent=String(i.map(t=>t.qty).reduce((t,e)=>t+e,0)),localStorage.setItem("Shopping cart",JSON.stringify(i)),j()},u=()=>{q(),B(),S()},S=()=>{s("#cart-total").textContent=`${p}kr`},j=()=>{p=[0,...i.map(t=>t.price*t.qty)].reduce((t,e)=>e+=t),localStorage.setItem("Total price",JSON.stringify(p))},N=async t=>(await x()).data.find(r=>t===r.id),g=t=>i.find(e=>e.id===t),B=()=>{s("#cart-list").innerHTML=i.map(t=>`
            <li class="cart-item">
        ...
        `).join(""),i.length===0?s("#checkout-btn").setAttribute("disabled","disabled"):s("#checkout-btn").removeAttribute("disabled")};s("#cart-list").addEventListener("keyup",t=>{const e=t.target,r=Number(e.dataset.inputId);if(!r)return;const o=g(r),a=s(`#input-${r}`);o.qty=Number(a.value),q();const c=s(`#item-price-${r}`);c.textContent=`${o.price*o.qty} kr`,S()});s("#cart-list").addEventListener("focusout",t=>{const e=t.target,r=Number(e.dataset.inputId);if(!r)return;const o=g(r);if(!(o.qty>0))i.splice(i.indexOf(o),1),u();else if(o.qty>o.stock_quantity){const a=s(`#input-${r}`);a.value=String(o.stock_quantity),o.qty=Number(a.value),console.log(o.qty),u(),I(o)}});s("#cart-list").addEventListener("click",async t=>{const e=t.target,r=Number(e.dataset.productId);if(!r)return;const o=g(r);e.tagName==="svg"?o.qty=0:e.className.includes("increase")?E(o):e.className.includes("decrease")&&o.qty--,o.qty>0||i.splice(i.indexOf(o),1),u()});const D=async()=>{d("#spinner");try{m=await x(),U()}catch{s("#output").innerHTML='<h2 class="nav-item px-2">🚨 KUNDE INTE HÄMTA DATA FRÅN SERVER 🚨 <br> försök igen senare...</h2>',s("#main").innerHTML='<h2 class="p-5">❌</h2>'}l("#spinner")},U=()=>{const t=m.data.map(e=>e.stock_status).filter(e=>e==="instock").length;s("#output").innerHTML=`Vi har ${t} st av ${m.data.length} st produkter i lager`,m.data.sort((e,r)=>e.name.localeCompare(r.name)),s(".product-main").innerHTML=m.data.map(e=>`
    ...
    `).join("")},I=t=>{s("#no-more-candy").innerHTML=`<p>${t.name}<br> är inte längre tillgängligt.</p>`,d("#no-more-candy"),setTimeout(()=>{l("#no-more-candy")},2e3)},C=t=>{const e=t.id,r=i.map(a=>a.id),o=g(e);!o||!r.includes(e)?(t.qty=1,i.push(t)):r.includes(e)&&E(o),s("#title-cart").classList.add("shake"),setTimeout(()=>{s("#title-cart").classList.remove("shake")},950)},E=t=>{if(t.stock_quantity>t.qty)t.qty++;else{I(t);return}};s("main").addEventListener("click",async t=>{var a;const e=t.target,r=Number(e.dataset.productId),o=await N(r);(a=e.getAttribute("class"))!=null&&a.includes("product-click-event")&&(e.tagName==="BUTTON"?(C(o),u()):(F(o),document.body.style.overflow="hidden"))});s("#title-cart").addEventListener("click",()=>{s(".cart-background").classList.add("show"),document.body.style.overflow="hidden"});s("#cart-close").addEventListener("click",()=>{s(".cart-background").classList.remove("show"),document.body.style.removeProperty("overflow")});s("#clear-cart-btn").addEventListener("click",async()=>{localStorage.removeItem("Shopping cart"),f=localStorage.getItem("Shopping cart")??"[]",i=JSON.parse(f),u(),setTimeout(()=>{s(".cart-background").classList.remove("show"),document.body.style.removeProperty("overflow")},500)});const F=t=>{d(".info-background"),s(".info-background").classList.add("show-info"),s("#info-section").innerHTML=`    
    ...
    `};s(".info-background").addEventListener("click",async t=>{const e=t.target,r=Number(e.dataset.productId),o=await N(r);e.tagName==="BUTTON"?(C(o),u(),document.body.style.removeProperty("overflow")):(e.tagName==="svg"||e.tagName==="path"||e.className.includes("info-background"))&&(l(".info-background"),document.body.style.removeProperty("overflow"))});const G=()=>{window.scrollTo(0,0),window.scrollTo(0,0),l(".content-display"),l("#title-cart"),l("#main"),l("footer"),d(".order-content-wrap"),d(".customer-details"),d(".back-button"),s(".content-wrapper").classList.add("banner-checkout"),s(".cart-background").classList.remove("show"),i.map(t=>{document.body.style.removeProperty("overflow");let e=t.price*t.qty;s("#order-content").innerHTML+=`
    ...
    `},J=()=>{d(".customer-details"),b.value=n.customer_first_name??"",k.value=n.customer_last_name??"",y.value=n.customer_address??"",h.value=n.customer_postcode??"",w.value=n.customer_city??"",$.value=n.customer_phone??"",L.value=n.customer_email??""},z=()=>{n={customer_first_name:b.value,customer_last_name:k.value,customer_address:y.value,customer_postcode:h.value,customer_city:w.value,customer_phone:$.value,customer_email:L.value};const t=JSON.stringify(n);localStorage.setItem("Customer data",t)};s("#checkout-btn").addEventListener("click",()=>{G(),J(),s("#checkout-btn").blur()});const V=()=>{document.querySelector(".order-confirmation").innerHTML=`
        <div class="alert alert-danger">Your order could not be placed. Please try again</div>
    `};let _=!1;P.addEventListener("submit",async t=>{t.preventDefault(),z(),_=!0,_&&s(".send-order").setAttribute("disabled","disabled");const e=i.map(a=>({product_id:a.id,qty:a.qty,item_price:a.price,item_total:a.price*a.qty})),r={customer_first_name:b.value,customer_last_name:k.value,customer_address:y.value,customer_postcode:h.value,customer_city:w.value,customer_email:L.value,customer_phone:$.value,order_total:p,order_items:e};await(async()=>{window.scrollTo(0,0);const a=await A(r);try{l(".checkout-wrap"),l(".back-button"),s(".order-confirmation").innerHTML+=`
