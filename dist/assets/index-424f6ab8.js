(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const a of document.querySelectorAll('link[rel="modulepreload"]'))o(a);new MutationObserver(a=>{for(const c of a)if(c.type==="childList")for(const v of c.addedNodes)v.tagName==="LINK"&&v.rel==="modulepreload"&&o(v)}).observe(document,{childList:!0,subtree:!0});function r(a){const c={};return a.integrity&&(c.integrity=a.integrity),a.referrerpolicy&&(c.referrerPolicy=a.referrerpolicy),a.crossorigin==="use-credentials"?c.credentials="include":a.crossorigin==="anonymous"?c.credentials="omit":c.credentials="same-origin",c}function o(a){if(a.ep)return;a.ep=!0;const c=r(a);fetch(a.href,c)}})();const T="https://www.bortakvall.se/api",E="/products",O="/orders",x=async()=>{const t=await fetch(`${T}${E}`);if(!t.ok)throw document.querySelector("#nav-output").innerHTML=`<h2 class="nav-item px-2">${t.status} ${t.statusText}</h2>`,new Error(`Could not get data, reason: ${t.status} ${t.statusText}`);return await t.json()},M=async t=>{const e=await fetch(`${T}${O}`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(t)});if(!e.ok)throw new Error(`${e.status} ${e.statusText}`);return await e.json()},s=t=>document.querySelector(`${t}`),l=t=>s(t).classList.add("d-none"),d=t=>s(t).classList.remove("d-none"),A=s(".customer-details"),b=s("#customer-first-name"),k=s("#customer-last-name"),h=s("#customer-address"),y=s("#customer-postal-number"),w=s("#customer-city"),$=s("#customer-phone"),L=s("#customer-email");let m,f=localStorage.getItem("Shopping cart")??"[]",i=JSON.parse(f),P=localStorage.getItem("Total price")??"0",p=JSON.parse(P),H=localStorage.getItem("Customer data")??"[]",n=JSON.parse(H);const S=()=>{s("#cart-item-count").textContent=String(i.map(t=>t.qty).reduce((t,e)=>t+e,0)),localStorage.setItem("Shopping cart",JSON.stringify(i)),R()},u=()=>{S(),j(),q()},q=()=>{s("#cart-total").textContent=`${p}kr`},R=()=>{p=[0,...i.map(t=>t.price*t.qty)].reduce((t,e)=>e+=t),localStorage.setItem("Total price",JSON.stringify(p))},I=async t=>(await x()).data.find(r=>t===r.id),g=t=>i.find(e=>e.id===t),j=()=>{s("#cart-list").innerHTML=i.map(t=>`
            ...
            <div class="card product-wrap border-0">
                ...
                <div class="card-body">
                    ...
                    <p class="info-icon-wrap product-click-event">     
                        <svg xmlns="http://www.w3.org/2000/svg" class="product-click-event bi-info-square info-icon" data-product-id="${e.id}" width="35" height="35" fill="currentColor" viewBox="0 0 16 16">
    ...
    `};s(".info-background").addEventListener("click",async t=>{const e=t.target,r=Number(e.dataset.productId),o=await I(r);e.tagName==="BUTTON"?(N(o),u(),document.body.style.removeProperty("overflow")):(e.tagName==="svg"||e.tagName==="path"||e.className.includes("info-background"))&&(l(".info-background"),document.body.style.removeProperty("overflow"))});const J=()=>{l(".content-display"),l("#title-cart"),l("#main"),l("footer"),d(".order-content-wrap"),d(".customer-details"),d(".back-button"),s(".content-wrapper").classList.add("banner-checkout"),s(".cart-background").classList.remove("show"),i.map(t=>{document.body.style.removeProperty("overflow");let e=t.price*t.qty;s("#order-content").innerHTML+=`
    ...
    `},z=()=>{d(".customer-details"),b.value=n.customer_first_name??"",k.value=n.customer_last_name??"",h.value=n.customer_address??"",y.value=n.customer_postcode??"",w.value=n.customer_city??"",$.value=n.customer_phone??"",L.value=n.customer_email??""},F=()=>{n={customer_first_name:b.value,customer_last_name:k.value,customer_address:h.value,customer_postcode:y.value,customer_city:w.value,customer_phone:$.value,customer_email:L.value};const t=JSON.stringify(n);localStorage.setItem("Customer data",t)};s("#checkout-btn").addEventListener("click",()=>{J(),z(),s("#checkout-btn").blur()});const V=()=>{document.querySelector(".order-confirmation").innerHTML=`
        <div class="alert alert-danger">Your order could not be placed. Please try again</div>
    `};let _=!1;A.addEventListener("submit",async t=>{t.preventDefault(),F(),_=!0,_&&s(".send-order").setAttribute("disabled","disabled");const e=i.map(a=>({product_id:a.id,qty:a.qty,item_price:a.price,item_total:a.price*a.qty})),r={customer_first_name:b.value,customer_last_name:k.value,customer_address:h.value,customer_postcode:y.value,customer_city:w.value,customer_email:L.value,customer_phone:$.value,order_total:p,order_items:e};await(async()=>{const a=await M(r);try{l(".checkout-wrap"),l(".back-button"),s(".order-confirmation").innerHTML+=`
